# 👋 Hi, I'm Mehfooz-ur-Rehman

<p align="left">
  <strong>Full-Stack Product Engineer & Founder</strong> • 6+ Years Software Development Experience (Devscot, CrazyBeings, LeadProfit) • Faisalabad / Samundri, Pakistan
</p>

<p align="left">
  <a href="https://mehfooz-ur-rehman.web.app"><img src="https://img.shields.io/badge/Portfolio-Live_Showcase-059669?style=flat-square&logo=googlechrome&logoColor=white" alt="Portfolio" /></a>
  <a href="https://github.com/MehfoozurRehman"><img src="https://img.shields.io/badge/GitHub-Public_Projects-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" /></a>
  <a href="mailto:mehfooz@devscot.com"><img src="https://img.shields.io/badge/Email-Get_in_Touch-EA4335?style=flat-square&logo=gmail&logoColor=white" alt="Email" /></a>
</p>

---

## 🚀 About Me

I design, architect, and ship high-velocity digital products, local-first desktop apps, real-time enterprise platforms, and AI workflows. With over **6+ years of software development experience**, I specialize in taking products from zero to production scale — prioritizing pragmatic architecture, offline tolerance, and friction-free user experiences.

- 🏗️ **Founder & Lead Architect** at [Karobari](https://github.com/MehfoozurRehman/karobari) — An AI-powered business operating system and digital storefront generator for Pakistani merchants built around conversational WhatsApp voice/text interactions.
- ⚡ **Real-Time & Local-First Engineering**: Building reactive cloud systems (**Convex**, Supabase), desktop workstations (**Electron** + React 19), and cross-platform mobile apps (**React Native / Expo**).
- 🏢 **Multi-System Engineering**: Extensive experience building ERP suites, point-of-sale terminals, agency platforms, and enterprise hardware/API bridges.

---

## 🛠️ Core Engineering Stack

```text
Frontend & UI        : Next.js 16 (App Router), React 19, TypeScript, Tailwind CSS v4, Motion (Framer Motion)
Real-Time & Backend  : Convex, Node.js, Express, Hono, REST / GraphQL / WebSockets, Prisma ORM, MongoDB, PostgreSQL
Mobile & Desktop     : React Native, Expo SDK, Electron 42, Zustand, Local-First Architecture
AI & Workflows       : Vercel AI SDK, OpenAI Whisper (Speech-to-Text), Google Gemini, Multi-Agent LLM Orchestration
Integrations         : WhatsApp Business Cloud API, Meta Graph API, Resend, Paddle Billing, JazzCash / EasyPaisa
```

---

## 📈 Engineering Principles

- **Pragmatic Over Complex**: Choose battle-tested, high-leverage primitives (Convex real-time queries, Next.js Server Components, local-first Zustand state).
- **Offline-First & Fault-Tolerant**: Build systems that keep functioning regardless of network drops or API throttling.
- **Product-Minded Execution**: Always align technical decisions with real customer usability and localized payment/operational habits.

---

<p align="center">
  <sub>Crafted with passion by <a href="https://github.com/MehfoozurRehman">Mehfooz-ur-Rehman</a>. Available for high-impact product engineering and advisory.</sub>
</p>

